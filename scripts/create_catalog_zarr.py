...
recipe_id = os.environ[
    "RECIPE_ID"
]  # id rather fail here then have weird 'None...' stores
...
if (
    "data_store_path" in recipe_meta.keys()
    and "data_store_prefix" in catalog_meta.keys()
):
    # raise error if both are present
    raise ValueError(
        "Both 'data_store_path' and 'data_store_prefix' are present in the recipe metadata. Please only specify one."
    )
