...
def find_recipe_meta(catalog_meta: List[Dict[str, str]], recipe_id:str)->Dict[str,str]:
    # Iterate over each dictionary in the list
    for d in catalog_meta:
        # Check if the 'id' key matches the search_id
        if d['id'] == recipe_id:
            return d
    print(f"Could not find {recipe_id=}. Got the following recipe_ids: {[d['id'] for d in catalog_meta]}")
...
injection_attrs['pangeo_forge_build_git_hash'] = git_url_hash
injection_attrs['pangeo_forge_build_timestamp'] = timestamp
